...
# ⚡ حل مشكلة EmptyDataError: التأكد من وجود العناوين دائماً
...
# ⚡ حل مشكلة الـ SyntaxError في بيانات الموظفين
if 'users_df' not in st.session_state:
    if not os.path.exists(users_file) or os.stat(users_file).st_size == 0:
        ...
        for u in initial_users:
            ...
            pwd = 'admin123' if u == 'jsafaa' else ('manager123' if u == 'ahatim' else '123')
            user_data.append({"username": u, "password": pwd, "name": u.upper(), "role": role, "first_login": True})
...
# --- 📤 Excel Export Function (حل مشكلة xlsxwriter) ---
def to_excel(df):
    output = io.BytesIO()
...
if st.session_state["authentication_status"]:
    ...
    st.sidebar.markdown(f'<div class="user-name-sidebar">👤 {user_info["name"]} ({user_info["role"]})</div>', unsafe_allow_html=True)
    ...
    if username in ['jsafaa', 'ahatim']:
        ...
        with c1: st.markdown(f'<div class="stat-card" style="background-color:#e3f2fd; border-color:#1565c0;"><div class="stat-label" style="color:#1565c0;">Total</div><div class="stat-value">{len(df)}</div></div>', unsafe_allow_html=True)
        with c2: 
    ...
    # Tabs
    tabs = st.tabs(["📊 Main System", "👥 Manage Staff"]) if username == 'jsafaa' else [st.container()]
    
    with tabs[0]:
        # عرض البيانات وتعديلها (كما في الكود السابق)
        st.dataframe(st.session_state.df_appeals, use_container_width=True)
        if st.button("📥 Download Report (Excel)"):
            data = to_excel(st.session_state.df_appeals)
            st.download_button("Click to Download", data, "NMC_Report.xlsx")

    if username == 'jsafaa':
        with tabs[1]:
            st.subheader("👥 Manage Staff")
            # هنا تضع كود الإضافة والمسح وتغيير الباسوورد (كما في الرد السابق)
            st.info("Everything is working now!")

elif st.session_state["authentication_status"] is False: st.error("Invalid credentials")
